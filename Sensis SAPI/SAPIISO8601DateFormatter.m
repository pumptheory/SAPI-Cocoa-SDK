 ...
 *
 * Canonical version: https://bitbucket.org/boredzo/iso-8601-parser-unparser
 *
 */
...
#endif
unichar SAPIISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface SAPIISO8601DateFormatter(UnparsingPrivate)
...
@implementation SAPIISO8601DateFormatter
...
- (id) init {
	if ((self = [super init])) {
		...
		format = SAPIISO8601DateFormatCalendar;
		timeSeparator = SAPIISO8601DefaultTimeSeparatorCharacter;
		includeTime = NO;
		parsesStrictly = NO;
	}
	return self;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	if (strict) timeSep = SAPIISO8601DefaultTimeSeparatorCharacter;
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case SAPIISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case SAPIISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case SAPIISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %d, not calendar (%d), week (%d), or ordinal (%d)", self.format, SAPIISO8601DateFormatCalendar, SAPIISO8601DateFormatWeek, SAPIISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = SAPIISO8601DefaultTimeSeparatorCharacter;
